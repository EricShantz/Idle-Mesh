...
- **Hidden upgrades**: `UpgradeDef` has optional `hidden?: boolean`. Hidden upgrades are filtered out of `NodeModal` and `NodeCard` badge counts. Used for `topicFilterBoost` until the topic system is implemented. Additionally, `subscriptionBroaden` is conditionally hidden when the queue has no `subscriptionTopic` (only visible after connecting to a broker).
- **Topic subscription picker**: `getAvailableTopics(queueId)` in `gameStore.ts` walks broker connections (including bridges) to find all reachable publishers, then generates one entry per publisher broadened to the queue's current `subscriptionBroaden` level. Deduplicates by broadened topic string — at high broaden levels, multiple publishers collapse into the same pattern. `setQueueSubscription(queueId, topic, segments, broadenLevel)` updates the queue's `subscriptionSegments`, `subscriptionTopic`, and `subscriptionBroaden` level atomically. UI lives in `NodeModal.tsx`: a "Subscription" header shows the current topic, with a "Change Topic" button that expands an inline picker (hidden when ≤1 option). Current topic highlighted in amber. Picker state resets on node switch via `key={node.id}` on the modal's motion.div.
