...
- Three passes per frame: Pass 1 (main dot loop: travel, collide, drop, DMQ catch), Pass 2 (queue auto-release, skips DMQ-queued dots), Pass 3 (DMQ auto-release: only if no retry dot is still on the DMQ→broker segment, rebuilds path from current positions).
...
- **Drag-to-move**: implemented in `NodeCard.tsx` using pointer capture + ref-based drag state. State update happens on every `pointermove` (via `moveComponent` in `gameStore.ts`) so connection lines and event canvas follow in real-time. Gear button and output port have `onPointerDown` stopPropagation to prevent drag-start. `draggingNodeId` (transient, in store) is set when drag movement begins and cleared on pointer up — Pass 2 and Pass 3 skip releasing dots from the dragged queue/DMQ to prevent mass release during repositioning. On release, queued dots have their queue→subscriber path segment rebuilt from current positions (truncate at queue waypoint, regenerate orthogonal midpoints to subscriber) so dots follow the updated connection line.
