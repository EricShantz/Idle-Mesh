...
- Three passes per frame: Pass 1 (main dot loop: travel, collide, drop, DMQ catch, live path rebuild during drag), Pass 2 (queue auto-release, skips DMQ-queued dots), Pass 3 (DMQ predictive auto-release: if first target on retry path is a queue, releases when queue has buffer space (counting both queued and in-flight dots); if subscriber, uses same timing prediction as Pass 2. Rebuilds path from current positions).
