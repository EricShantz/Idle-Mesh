...
- **Auto-release**: FIFO by `pauseStartTime` — oldest queued dot released first, one per frame, only when the queue has a current connection to a subscriber AND subscriber is free (no pausing dot, no traveling dot past any queue heading to that subscriber). Uses current connection graph, not baked paths. With fan-out (`fanOut` upgrade), waits until ALL connected subscribers are free, then sends a copy to each. Without fan-out, round-robins across connected subscribers (competing consumers).
