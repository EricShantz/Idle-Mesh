...
- Triggers: `intro` on first mount via `App.tsx`, `brokerUpgrade` inside `upgradeComponent`, component-type tutorials inside `addComponent`, `firstDrop` from `useGameLoop.ts` 1 second after the first event drop. Publisher/subscriber tutorials trigger on 2nd instance.
